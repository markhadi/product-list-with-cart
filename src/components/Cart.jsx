...
const Cart = () => {
  const { cart, modifyCart, getCartTotal } = useCart();
  const orderTotal = getCartTotal();
  const [modalStatus, setModalStatus] = useState(null);

  const handleConfirmOrder = () => setModalStatus("open");
  const handleCloseModal = () => {
    setModalStatus("closing");
    setTimeout(() => setModalStatus(null), 400);
  };

  return (
    <div className="bg-white rounded-md p-6 h-max lg:max-w-[384px]">
      <h2 className="font-bold text-[24px] text-red">Your Cart ({Object.keys(cart).length})</h2>

      {Object.keys(cart).length === 0 ? (
        <div className="flex-center flex-col gap-4 mt-11">
          ...
          <p className="font-semibold text-[14px] text-rose-500">Your added items will appear here</p>
        </div>
      ) : (
        <div className="mt-6 flex flex-col gap-4">
          {Object.values(cart).map((item) => (
            <CartItem key={item.id} item={item} removeFromCart={() => modifyCart(item, "remove")} />
          ))}

          <div className="mt-7">
            <div className="flex items-center justify-between mb-6 text-rose-900">
              <span className="text-[14px] font-normal">Order Total</span>
              <span className="text-[24px] font-bold">${orderTotal.toFixed(2)}</span>
            </div>
            <div className="flex-center gap-2 h-[52px] w-full bg-rose-50 rounded-md mb-6">
            ...
            <button className="btn-order" onClick={handleConfirmOrder}>
              Confirm Order
            </button>
          </div>
        </div>
      ...
      <OrderConfirmed onClose={handleCloseModal} className={modalStatus === "open" ? "active" : modalStatus === "closing" ? "closing" : ""} />
    </div>
  );
};
